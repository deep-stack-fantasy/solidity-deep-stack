...
)
{
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(2 + dynPointers))
					<< evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(2)) ;
			m_context << Instruction::SUB;
			m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(2 + dynPointers  - thisDynPointer));
			...
			else
			{
				...
				m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(1 + arrayType->sizeOnStack()));
				ArrayUtils(m_context).retrieveLength(*arrayType, 1);
				// stack: ... <end_of_mem> <value...> <end_of_mem'> <length>
				storeInMemoryDynamic(*TypeProvider::uint256(), true);
				// stack: ... <end_of_mem> <value...> <end_of_mem''>
				// copy the new memory pointer
				m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(arrayType->sizeOnStack() + 1)) << Instruction::POP;
				// stack: ... <end_of_mem''> <value...>
				// copy data part
				ArrayUtils(m_context).copyArrayToMemory(*arrayType, _padToWordBoundaries);
				// stack: ... <end_of_mem'''>
	...
	m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(argSize + dynPointers + 1));
			...
			if (!targetType || *sourceType != *targetType || _cleanupNeeded)
			{
				...
				if (sourceSize > 0 || targetSize > 0)
				{
					...
					for (unsigned j = 0; j < min(sourceSize, targetSize); ++j)
						m_context << AssemblyItem(Instruction::SWAPE, (uint8_t)(depth + targetSize - sourceSize))
								  << Instruction::POP;
					// Value shrank
...
void CompilerUtils::moveToStackVariable(VariableDeclaration const& _variable)
{
	...
	for (unsigned i = 0; i < size; ++i)
		m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(stackPosition - size + 1))
			<< Instruction::POP;
}
...
void CompilerUtils::copyToStackTop(unsigned _stackDepth, unsigned _itemSize)
{
	...
	for (unsigned i = 0; i < _itemSize; ++i)
		m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(_stackDepth));
}
...
void CompilerUtils::rotateStackUp(unsigned _items)
{
	...
	for (unsigned i = 1; i < _items; ++i)
		m_context << AssemblyItem(Instruction::SWAPE, (uint8_t)(_items - i));
}
