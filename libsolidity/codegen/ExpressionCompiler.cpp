...
void ExpressionCompiler::appendConstStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	// append return
	m_context <<evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(_varDecl.annotation().type->sizeOnStack() + 1));
...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	{
		m_context << AssemblyItem(Instruction::SWAPE, (uint8_t)(static_cast<unsigned>(paramTypes.size())));
		m_context << Instruction::POP;
		m_context << AssemblyItem(Instruction::SWAPE, (uint8_t)(static_cast<unsigned>(paramTypes.size())));
	...
	if (retSizeOnStack > DSF_MAX_STACK_ACCESS-1)
		BOOST_THROW_EXCEPTION(
			StackTooDeepError() <<
			errinfo_sourceLocation(_varDecl.location()) <<
			util::errinfo_comment(util::stackTooDeepString)
		);
	m_context <<evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(retSizeOnStack + 1));
	m_context.appendJump(evmasm::AssemblyItem::JumpType::OutOfFunction);
}
...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	else  // compound assignment
	{
		...
		if (lvalueSize > 0)
		{
			if (itemSize + lvalueSize > DSF_MAX_STACK_ACCESS)
				BOOST_THROW_EXCEPTION(
					StackTooDeepError() <<
			...
			for (unsigned i = 0; i < itemSize; ++i)
				m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(itemSize + lvalueSize)) << Instruction::POP;
		}
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::Creation:
		{
			...
			if (function.saltSet())
			{
				m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(2 + (function.valueSet() ? 1 : 0)));
			...
			if (function.valueSet())
				m_context <<evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(3 + (function.saltSet() ? 1 : 0)));
			else
				m_context << u256(0);
		...
		case FunctionType::Kind::SetGas:
		{
			...
			if (stackDepth > 0)
				m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(stackDepth));
			if (function.gasSet())
				m_context << Instruction::POP;
			break;
		}
...
)
{
	...
	{
		m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(2 + gasValueSize + CompilerUtils::sizeOnStack(argumentTypes)));
	...
	if (isDelegateCall)
		...
		m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(m_context.baseToCurrentStackOffset(valueStackPos)));
	else
		m_context << u256(0);
	m_context << evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(m_context.baseToCurrentStackOffset(contractStackPos)));
	...
	if (_functionType.gasSet())
		m_context <<evmasm::AssemblyItem(Instruction::DUPE, (uint8_t)(m_context.baseToCurrentStackOffset(gasStackPos)));
	...
	if (!returnSuccessConditionAndReturndata && !_tryCall)
	{
		...
		m_context <<AssemblyItem(Instruction::SWAPE, (uint8_t)(remainsSize));
