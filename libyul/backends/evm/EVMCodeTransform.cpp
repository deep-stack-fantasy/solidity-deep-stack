		...
		else
		{
			...
			for (auto it = m_unusedStackSlots.begin(); it != m_unusedStackSlots.end(); ++it)
			{
				...
				if (size_t heightDiff = variableHeightDiff(var, varName, true)){
					...
					m_assembly.appendData(bytes{(uint8_t)(static_cast<unsigned>(heightDiff - 1))});
				}
				m_assembly.appendInstruction(evmasm::Instruction::POP);
				break;
			}
...

	{
		...
		else
		{
			while (!stackLayout.empty() && stackLayout.back() != static_cast<int>(stackLayout.size() - 1))
				...
				{
					m_assembly.appendInstruction(evmasm::Instruction::SWAPE);
					m_assembly.appendData(bytes{(uint8_t)(static_cast<unsigned>(stackLayout.size()) - static_cast<unsigned>(stackLayout.back()) - 1u)});
